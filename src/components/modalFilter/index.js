...
class ModalFilter extends Component {
  ...
  removeField(itemId,values,setValues){
    const filters = [...values.filters];
    // let newFilter = filters.filter((item,id) => id !== itemId);
                    ...
                    values.filters.map((item, i) => {
                      return (
                        <Grid key={i} className="w-full p-1">
                          ...
                          <Grid.Col span={1}>
                            <button type='button' className='p-2 bg-red-500 rounded' onClick={() => this.removeField(i,values,setValues)}>
