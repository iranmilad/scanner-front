...
const PrivateSection = (props) => {
  ...
  useQuery({
    enabled: cookies.token !== undefined && cookies.token !== null && cookies.token !== '',
    ...
    staleTime: 60 * 1000,
    onSuccess: (data) => {
      // get unseen notifications by null value
      const unseen = data.data.filter(item => item.seen === null);
        ...
        <Menu
          ...
          control={
            ...
            disabled={pathname === "/dashboard"}
              size={16}
              label={notificationsCount <= 0 ? '' : notificationsCount}
              inline
              color={ notificationsCount <= 0 ? 'green' : 'red'}
              position="top-start"
              sh
              offset={3}
              withBorder
