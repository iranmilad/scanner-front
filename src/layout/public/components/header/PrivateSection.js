  ...
  render(){
    if(getLocalStorage('userToken')){
      return (
        ...
        <Menu dir="rtl" control={<Avatar src="https://randomuser.me/api/portraits/men/9.jpg" />}>
          <Menu.Item icon={<FiUser size={15} />}>تنظیمات حساب کاربری</Menu.Item>
          <Menu.Item icon={<FiKey size={15}/>}>اشتراک ویژه</Menu.Item>
          <Divider/>
          <Menu.Item color="red" icon={<FiLogOut size={15} />} onClick={()=> this.handleLogout()} >خروج</Menu.Item>
        </Menu>
      </div>
      )
    }
